...
-export([fresh/0, descends/2, merge/1, get_counter/2, subtract_dots/2,
         ...
         to_binary/1, from_binary/1, dominates/2, glb/2, update_val/3]).
...
% @doc update val of VClock at Node.
-spec update_val(Node :: vclock_node(),
                VClock :: vclock(), Val :: integer()) -> vclock().
update_val(Node, VClock, Val) ->
    {Ctr, NewV} = case lists:keytake(Node, 1, VClock) of
                                false ->
                                    {Val, VClock};
                                {value, {_N, _C}, ModV} ->
